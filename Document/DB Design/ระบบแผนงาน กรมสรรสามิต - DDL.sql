...
/**
 * View สรุปภาพรวมคำขอเงินงบประมาณ
 *
**/
IF OBJECT_ID('dbo.V_GET_BUDGET_REQUEST_DETAIL_ONLY_DESCRIBE', N'V') IS NOT NULL 
	DROP VIEW dbo.V_GET_BUDGET_REQUEST_DETAIL_ONLY_DESCRIBE;
GO
CREATE VIEW V_GET_BUDGET_REQUEST_DETAIL_ONLY_DESCRIBE AS
SELECT D.*, M.REQ_TYPE, M.BUDGET_TYPE, M.REFER_REQ_ID, M.BUDGET_ALLOCATE_FLAG, M.OFF_BUDGET_ALLOCATE_FLAG
	, AREA_NAME = (SELECT AREA_NAME FROM T_AREA AREA WHERE AREA.AREA_ID = M.AREA_ID)
	, DEP_NAME = (SELECT DEP_NAME FROM T_DEPARTMENT DEP WHERE DEP.DEP_ID = D.DEP_ID)
	, DEP_ORDER_SEQ = (SELECT DEP.SORT_INDEX FROM T_DEPARTMENT DEP WHERE DEP.DEP_ID = D.DEP_ID)

	-- ชื่อค่าใช้จ่าย
	, (SELECT PLAN_NAME FROM T_PLAN_CONFIGURE P WHERE P.PLAN_ID = D.PLAN_ID) AS PLAN_NAME
	, (SELECT ORDER_SEQ FROM T_PLAN_CONFIGURE P WHERE P.PLAN_ID = D.PLAN_ID) AS PLAN_ORDER_SEQ

	, (SELECT PRODUCE_NAME FROM T_PRODUCE_CONFIGURE P WHERE P.PRODUCE_ID = D.PRODUCE_ID) AS PRODUCE_NAME
	, (SELECT ORDER_SEQ FROM T_PRODUCE_CONFIGURE P WHERE P.PRODUCE_ID = D.PRODUCE_ID) AS PRODUCE_ORDER_SEQ

	, (SELECT ACTIVITY_NAME FROM T_ACTIVITY_CONFIGURE A WHERE A.ACTIVITY_ID = D.ACTIVITY_ID) AS ACTIVITY_NAME
	, (SELECT ORDER_SEQ FROM T_ACTIVITY_CONFIGURE A WHERE A.ACTIVITY_ID = D.ACTIVITY_ID) AS ACTIVITY_ORDER_SEQ

	, (SELECT BUDGET_TYPE_NAME FROM T_BUDGET_TYPE B WHERE B.BUDGET_TYPE_ID = D.BUDGET_TYPE_ID) AS BUDGET_TYPE_NAME
	, (SELECT ORDER_SEQ FROM T_BUDGET_TYPE B WHERE B.BUDGET_TYPE_ID = D.BUDGET_TYPE_ID) AS BUDGET_TYPE_ORDER_SEQ

	, EXPENSES_GROUP.EXPENSES_MASTER_ID
	, EXPENSES_GROUP.EXPENSES_MASTER_NAME
	, EXPENSES_GROUP.EXPENSES_GROUP_NAME
	, EXPENSES_GROUP.ORDER_SEQ AS EXPENSES_GROUP_ORDER_SEQ

	, EXPENSES.EXPENSES_NAME AS EXPENSES_NAME
	, EXPENSES.ORDER_SEQ AS EXPENSES_ORDER_SEQ
	, EXPENSES.FORM_TEMPLATE_NAME
FROM T_BUDGET_REQUEST_DETAIL D, T_BUDGET_REQUEST_MASTER M, T_EXPENSES_ITEM EXPENSES
, (
	SELECT A.*
	, EXPENSES_MASTER_NAME = (SELECT EXPENSES_MASTER_NAME FROM T_EXPENSES_MASTER B WHERE B.EXPENSES_MASTER_ID = A.EXPENSES_MASTER_ID)
	FROM T_EXPENSES_GROUP A
) AS EXPENSES_GROUP
WHERE D.REQ_ID = M.REQ_ID
 AND M.ACTIVE = 1 AND D.ACTIVE = 1
 AND EXPENSES.EXPENSES_ID = D.EXPENSES_ID
 AND EXPENSES_GROUP.EXPENSES_GROUP_ID = D.EXPENSES_GROUP_ID
 AND D.EXPENSES_XML_DESCRIBE.value('(/root/root)[1]', 'nvarchar(max)') IS NOT NULL
GO
sp_refreshview @viewname = 'V_GET_BUDGET_REQUEST_DETAIL_ONLY_DESCRIBE'
GO
...
CREATE VIEW V_GET_SUMMARY_BUDGET_ALLOCATE AS
	...
	, EXPEN_ALLOCATE.PROJECT_ID as ALLOCATE_PROJECT_ID
	...
	LEFT JOIN (
		...
		LEFT JOIN (SELECT * FROM T_BUDGET_ALLOCATE_EXPENSES_GROUP WHERE ACTIVE = 1) AS ALLOCATE_GRP
...
CREATE VIEW V_GET_BUDGET_RESERVE_INFORMATION AS
	...
	, EXPEN_GROUP.EXPENSES_MASTER_ID, EXPEN_GROUP.EXPENSES_MASTER_NAME
	, EXPEN_GROUP.EXPENSES_GROUP_NAME
	, EXPEN_GROUP.ORDER_SEQ AS EXPENSES_GROUP_ORDER_SEQ
	...
	FROM T_BUDGET_RESERVE A
	JOIN V_GET_EXPENSES_GROUP_INFORMATION EXPEN_GROUP
		ON(EXPEN_GROUP.EXPENSES_GROUP_ID = A.EXPENSES_GROUP_ID)
...
CREATE VIEW V_GET_SUMMARY_BUDGET_RESERVE_INFORMATION AS
	...
	, EXPENSES_NAME = (SELECT EXPENSES_NAME FROM T_EXPENSES_ITEM E WHERE E.EXPENSES_ID = A.EXPENSES_ID)
	, EXPENSES_ORDER_SEQ = (SELECT ORDER_SEQ FROM T_EXPENSES_ITEM E WHERE E.EXPENSES_ID = A.EXPENSES_ID)
	...
	FROM (SELECT A.*, B.EXPENSES_MASTER_ID, B.EXPENSES_MASTER_NAME, B.EXPENSES_GROUP_NAME, B.ORDER_SEQ AS EXPENSES_GROUP_ORDER_SEQ FROM T_BUDGET_RESERVE A, V_GET_EXPENSES_GROUP_INFORMATION B WHERE A.ACTIVE = 1 AND A.EXPENSES_GROUP_ID = B.EXPENSES_GROUP_ID) AS A
...
AS
	...
	--BEGIN
		...
		FROM(
			...
			, TEMPLATE.PLAN_ID, TEMPLATE.PLAN_NAME, TEMPLATE.PLAN_ORDER_SEQ
			, TEMPLATE.PRODUCE_ID, TEMPLATE.PRODUCE_NAME, TEMPLATE.PRODUCE_ORDER_SEQ
			, TEMPLATE.ACTIVITY_ID, TEMPLATE.ACTIVITY_NAME, TEMPLATE.ACTIVITY_ORDER_SEQ
		
			, TEMPLATE.BUDGET_TYPE_ID, TEMPLATE.BUDGET_TYPE_NAME
			, (SELECT GOVERNMENT_REFER_CODE FROM T_BUDGET_TYPE_GOVERNMENT_REFER_CODE E WHERE E.BUDGET_TYPE_ID = TEMPLATE.BUDGET_TYPE_ID AND E.YR = @p_fiscal_year) AS BUDGET_TYPE_GOVERNMENT_REFER_CODE
			, TEMPLATE.BUDGET_TYPE_ORDER_SEQ
		
			, TEMPLATE.EXPENSES_GROUP_ID, TEMPLATE.EXPENSES_GROUP_NAME
			, (SELECT GOVERNMENT_REFER_CODE FROM T_EXPENSES_GROUP_GOVERNMENT_REFER_CODE E WHERE E.EXPENSES_GROUP_ID = TEMPLATE.EXPENSES_GROUP_ID AND E.YR = @p_fiscal_year) AS EXPENSES_GROUP_GOVERNMENT_REFER_CODE
			, TEMPLATE.EXPENSES_GROUP_ORDER_SEQ
		
			, TEMPLATE.EXPENSES_ID, TEMPLATE.EXPENSES_NAME
			, TEMPLATE.EXPENSES_ORDER_SEQ
			, (SELECT CAN_ADD_PROJECT FROM T_EXPENSES_ITEM E WHERE E.EXPENSES_ID = TEMPLATE.EXPENSES_ID) AS CAN_ADD_PROJECT
			...
			FROM (
				-- Template ที่นำไปใช้ได้กับทุกปีงบประมาณ
				SELECT * 
				FROM V_GET_BUDGET_TEMPLATE_EXPENSES_INFORMATION
				WHERE SHARED_YR_TEMPLATE = 1

				UNION ALL

				-- Template ที่นำไปใช้เฉพาะบางปีงบประมาณ ซึ่งต้องตรงกับปีงบประมาณที่จะแสดงข้อมูล @p_fiscal_year
				SELECT * 
				FROM V_GET_BUDGET_TEMPLATE_EXPENSES_INFORMATION TEMPLATE_MAS
				WHERE SHARED_YR_TEMPLATE = 2
				  AND EXISTS(
					SELECT * 
					FROM T_BUDGET_REQUEST_TEMPLATE_YR_AUTHORIZE YR_AUTHORIZE
					WHERE YR_AUTHORIZE.TEMPLATE_ID = TEMPLATE_MAS.TEMPLATE_ID
					  AND YR_AUTHORIZE.YR = @p_fiscal_year
				  )
			) as TEMPLATE
			WHERE 
			  ...
			  NOT EXISTS(
				SELECT *
				FROM T_BUDGET_EXPENSES BUDGET_EX
				WHERE BUDGET_EX.YR = @p_fiscal_year
				  AND BUDGET_EX.PLAN_ID = TEMPLATE.PLAN_ID
				  AND BUDGET_EX.PRODUCE_ID = TEMPLATE.PRODUCE_ID
				  AND BUDGET_EX.ACTIVITY_ID = TEMPLATE.ACTIVITY_ID
				  AND BUDGET_EX.BUDGET_TYPE_ID = TEMPLATE.BUDGET_TYPE_ID
				  AND BUDGET_EX.EXPENSES_GROUP_ID = TEMPLATE.EXPENSES_GROUP_ID
				  AND BUDGET_EX.EXPENSES_ID = TEMPLATE.EXPENSES_ID
			  )
		) AS REQ_EXPENSES
		;

		-- ยกเลิก เมื่อวันที่ 12/09/2021 ให้ไปใช้ข้อมูลจาก Template แทน
		-- เนื่องจากไม่รองรับในกรณีที่ ยังไม่มีคำขอจากหน่วยงานภูมิภาคมา แล้วไม่สามารถลงรายการงบประมาณ
		-- ที่รัฐบาลจัดสรรมาได้
		--
		--
