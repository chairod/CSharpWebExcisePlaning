...
$(document).ready(function () {
    $("#pcoded").pcodedmenu({
        ...
        LHeaderBackground: 'theme9',
        NavbarBackground: 'theme1',
        ActiveItemBackground: 'theme9',
    ...
    // กำหนดการแสดงผลหน้าเว็บไซด์ Logo
    // ถ้า #pcoded attribute vertical-nav-type="collapsed"
    // ไม่ต้องแสดง Logo
    function redrawUI() {
        var verticalNavType = $("#pcoded").attr('vertical-nav-type');
        var deviceType = $('#pcoded').attr('pcoded-device-type');
        if ("collapsed" == verticalNavType)
            $('#appLogo').addClass('d-none');
        else {
            $('#appLogo').removeClass('d-none');

            // กรณีเป็น phone ไม่ต้องแสดงรูปภาพ หรือ ข้อความ ในส่วนของ Logo
            if (deviceType == 'phone')
                $('#appLogo > image, #appLogo > span').hide();
        }


        // กำหนด Margin ของเนื้อหากรณี device เป็น phone
        if ('phone' == deviceType)
            $('.main-body').css('margin-top', '57px');
        else
            $('.main-body').css('margin-top', '');
    }
    ...
    // Bell Alert
    var bellAlertMenuEl = $("#bell-alert-menu");
    var hideAlertMenuId = null;
    $("#bell-alert,#bell-alert-menu").on("mouseenter", function () {
        clearTimeout(hideAlertMenuId);
        bellAlertMenuEl.removeClass("d-none");
        bellAlertMenuEl.addClass("animated fadeInUp");
    }).on("mouseleave", function () {
        hideAlertMenu();
    });
    function hideAlertMenu() {
        hideAlertMenuId = setTimeout(function () {
            bellAlertMenuEl.removeClass("animated fadeInUp");
            bellAlertMenuEl.addClass("d-none");
        }, 1000);
    }
});
